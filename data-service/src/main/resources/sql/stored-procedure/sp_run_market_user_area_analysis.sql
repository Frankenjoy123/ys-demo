...
inner join lu_province_city l on (l.city = usr.city  or l.city = concat(usr.city,'市') or (length(usr.city) >0 and l.city like concat(usr.city,'%')) )
...
where date(convert_tz(mr.created_datetime,'+00:00','+08:00')) = chooseDate
and (usr.city is null or length(usr.city) = 0 or usr.city = '未知' or (not exists(select 1 from lu_province_city where city like concat(usr.city,'%'))));
