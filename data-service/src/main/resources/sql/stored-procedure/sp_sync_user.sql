...
SELECT u.id, null, org.id as 'org_id', org.name as 'org_name', u.name, u.phone, u.email, u.age, u.sex, u.gravatar_url, case when u.oauth_type_code = 'webchat' then u.oauth_openid else null END,
...
insert into sp_sync_user_tmp_user
SELECT null, usr.ysid, org.id as 'org_id', org.name as 'org_name', null, null, null, null, null, null, null,
...
on u.org_id = tmp.org_id and (u.user_id = tmp.user_id or u.ys_id = tmp.ys_id);
...
where usr.user_id is not null and not exists (select 1 from emr_user where user_id = usr.user_id and org_id = usr.org_id);


-- 插入ys_id
insert into emr_user (user_id, ys_id, org_id, org_name, name, phone, email,age, sex, gravatar_url, wx_openid, province, city, address,join_datetime,latest_event_ip,latest_event_device)
select user_id, ys_id, org_id, org_name, name, phone, email,age, sex, gravatar_url, wx_openid,
ifnull(l.province,usr.province), ifnull(l.city,usr.city), address,join_datetime, last_event_ip, case when usr.last_user_agent like '%iphone%' then 'iPhone'
 when usr.last_user_agent like '%android%' then 'Android'
 when usr.last_user_agent like '%windows%' then 'Windows'
 else '其他' end  from sp_sync_user_tmp_user usr
 left join lu_province_city l on (l.city = usr.city  or l.city = concat(usr.city,'市') )
where usr.ys_id is not null and not exists (select 1 from emr_user where ys_id = usr.ys_id and org_id = usr.org_id);
