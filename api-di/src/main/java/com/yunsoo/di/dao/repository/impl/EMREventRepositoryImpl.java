...
public class EMREventRepositoryImpl implements CustomEMREventRepository {
    ...
    private int[] query(String action, String orgId, String productBaseId, String province, String city, DateTime createdDateTimeStart, DateTime createdDateTimeEnd, int level) {
        String sql = "select count(1), count(distinct u.id) from di_event ev inner join di_user u on ev.org_id = u.org_id and (ev.user_id = u.user_id and ev.ys_id = u.ys_id) inner join lu_province_city pc ON ev.location_id=pc.id where ev.name= :eventName" +
                " and ev.org_id =:orgId ";
