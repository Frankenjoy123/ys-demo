(function () {
    ...
    app.controller("packageCtrl", ["$scope", "FileUploader", "packageService", function ($scope, FileUploader, packageService) {
        ...
        $('#package-step-wz').bootstrapWizard({
            tabClass		: 'wz-steps',
            nextSelector	: '.next',
            ...
            onInit : function(){
                $('#package-step-wz').find('.finish').hide().prop('disabled', true);
            },
            onTabShow: function(tab, navigation, index) {
                ...
                $('#package-step-wz').find('.progress-bar').css({width:$percent+'%', 'margin': 0 + 'px ' + margin + '%'});


                // If it's the last tab then hide the last button and show the finish instead
                if($current >= $total) {
                    $('#package-step-wz').find('.next').hide();
                    $('#package-step-wz').find('.finish').show();
                    $('#package-step-wz').find('.finish').prop('disabled', false);
                } else {
                    $('#package-step-wz').find('.next').show();
                    $('#package-step-wz').find('.finish').hide().prop('disabled', true);
                }
            }
        });
