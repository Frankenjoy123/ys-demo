(function () {
  ...
  app.factory('productBaseManageService', ['$http', function ($http) {
    return {
      getProductBases: function (orgId, fnSuccess) {
        if (orgId == '') {
          $http.get('/api/productbase').success(fnSuccess);
        }
        else {
          $http.get('/api/productbase?org_id=' + orgId).success(fnSuccess);
        }
  ...
  app.controller('ProductBaseManageCtrl', ['$scope', 'productBaseManageService', 'productBaseDataService', '$location', "$timeout", '$route', function ($scope, productBaseManageService, productBaseDataService, $location, $timeout, $route) {
    ...
    var table = {
      ...
      flush: function (callback) {
        productBaseManageService.getProductBases($scope.currOrg, function (data, status, headers) {
    ...
    $scope.dataTable = new $scope.utils.DataTable(table);
    ...
    productBaseManageService.getOrgList(function (data) {
      $scope.orgList = data;
    }, function(){});
